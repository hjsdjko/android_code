...
import com.design.appproject.ui.wupinleixing.ListFilterDialog
...
import com.design.appproject.databinding.WupinleixingcommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.wupinleixing.ListModel
import com.design.appproject.ui.wupinleixing.ListAdapter
 ...
 * 物品类型列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_WUPINLEIXING)
class ListFragment : BaseBindingFragment<WupinleixingcommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("物品类型","wupinleixing")/*查询索引*/

    override fun initEvent() {
        setBarTitle("物品类型",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("wupinleixing","新增"))|| Utils.isAuthFront("wupinleixing","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_WUPINLEIXING).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_WUPINLEIXING)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("wupinleixing",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_WUPINLEIXING,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun WupinleixingcommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("wupinleixing","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
        ...
        mIsBack.yes {
                mListModel.page("wupinleixing", requestParams)
        }.otherwise {
                mListModel.list("wupinleixing", requestParams)
        }
    }
}