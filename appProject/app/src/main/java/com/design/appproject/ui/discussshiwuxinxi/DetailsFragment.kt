...
import com.design.appproject.logic.viewmodel.discussshiwuxinxi.DetailsViewModel
import androidx.activity.viewModels
import com.youth.banner.adapter.BannerImageAdapter
import com.youth.banner.holder.BannerImageHolder
import com.alibaba.android.arouter.facade.annotation.Autowired
import com.alibaba.android.arouter.facade.annotation.Route
import com.design.appproject.databinding.DiscussshiwuxinxicommonDetailsLayoutBinding
import com.design.appproject.databinding.DiscussshiwuxinxicommonDetailsHeaderLayoutBinding
 ...
 * 失物信息评论表详情页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_DETAILS_DISCUSSSHIWUXINXI)
class DetailsFragment : BaseBindingFragment<DiscussshiwuxinxicommonDetailsLayoutBinding>() {
    ...
    private var mDiscussshiwuxinxiItemBean=DiscussshiwuxinxiItemBean()/*详情内容*/

    private val mCommentsAdatper by lazy {
        CommentsAdatper().apply {
            pageLoadMoreListener {
                mDetailsViewModel.comments("discussdiscussshiwuxinxi", mapOf("page" to it.toString(),"limit" to "10","refid" to mId.toString()))
            }
        }
    }

    private val mHeader by lazy {
        LayoutInflater.from(requireActivity()).inflate(R.layout.discussshiwuxinxicommon_details_header_layout, null,false)
    }
    private val mHeaderBinding:DiscussshiwuxinxicommonDetailsHeaderLayoutBinding by lazy {
        mHeader.getBinding()
    }

    @SuppressLint("SuspiciousIndentation")
    override fun initEvent() {
        setBarTitle("失物信息评论表详情页")
        ...
        binding.apply{
            ...
            srv.setOnRefreshListener {
                loadData()
                mDetailsViewModel.comments("discussdiscussshiwuxinxi", mapOf("page" to "1","limit" to "10","refid" to mId.toString()))
            }
    ...
    override fun initData() {
        ...
        mDetailsViewModel.infoLiveData.observeKt(errorBlock = {binding.srv.isRefreshing =false}) {
            it.getOrNull()?.let { info->
                ...
                mDiscussshiwuxinxiItemBean = info.data
    ...
    private fun loadData(){
        mDetailsViewModel.info("discussshiwuxinxi",mId.toString())
    }


    private fun DiscussshiwuxinxicommonDetailsHeaderLayoutBinding.setInfo(){
        refidTv.text = "${mDiscussshiwuxinxiItemBean.refid}"
        avatarurlIfv.load(requireActivity(),"${mDiscussshiwuxinxiItemBean.avatarurl}")
        nicknameTv.text = "${mDiscussshiwuxinxiItemBean.nickname}"
        contentTv.text = "${mDiscussshiwuxinxiItemBean.content}"
        replyTv.text = "${mDiscussshiwuxinxiItemBean.reply}"
    }
