...
import com.design.appproject.ui.shiquxinxi.ListFilterDialog
...
import com.design.appproject.databinding.ShiquxinxicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.shiquxinxi.ListModel
import com.design.appproject.ui.shiquxinxi.ListAdapter
 ...
 * 拾取信息列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_SHIQUXINXI)
class ListFragment : BaseBindingFragment<ShiquxinxicommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("物品名称","wupinmingcheng")/*查询索引*/
    ...
    override fun initEvent() {
        setBarTitle("拾取信息",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("shiquxinxi","新增"))|| Utils.isAuthFront("shiquxinxi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_SHIQUXINXI).navigation()
            }
            initSearch()
            initCategory()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_SHIQUXINXI)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("shiquxinxi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_SHIQUXINXI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun ShiquxinxicommonListLayoutBinding.initSearch() {
    ...
    private fun ShiquxinxicommonListLayoutBinding.initCategory() {
        mIsBack.yes {
            HomeRepository.page<WupinleixingItemBean>("wupinleixing", mapOf("page" to "1", "limit" to "100")).observeKt(errorBlock = {binding.srv.isRefreshing =false}) {
            it.getOrNull()?.let {
                setNavigator(tab,it.data.list)
            }
        } }.otherwise {
            HomeRepository.list<WupinleixingItemBean>("wupinleixing", mapOf("page" to "1", "limit" to "100")).observeKt(errorBlock = {binding.srv.isRefreshing =false}) {
            it.getOrNull()?.let {
                setNavigator(tab,it.data.list)
            }
        }
    ...
    private fun setNavigator(tab: LinearLayout,list:List<WupinleixingItemBean>){
        mCategoryList.addAll(list.map{it.wupinleixing})
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("wupinmingcheng","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("wupinmingcheng")
        }
        if(mCategoryList[mSelectedIndex]!="全部"){
            params.put("wupinleixing","%" + mCategoryList[mSelectedIndex] + "%" )
        }else{
            params.remove("wupinleixing")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
                mListModel.page("shiquxinxi", requestParams)
        }.otherwise {
                mListModel.list("shiquxinxi", requestParams)
        }
    }
}