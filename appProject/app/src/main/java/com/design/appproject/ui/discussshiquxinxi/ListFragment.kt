...
import com.design.appproject.ui.discussshiquxinxi.ListFilterDialog
...
import com.design.appproject.databinding.DiscussshiquxinxicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.discussshiquxinxi.ListModel
import com.design.appproject.ui.discussshiquxinxi.ListAdapter
 ...
 * 拾取信息评论表列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_DISCUSSSHIQUXINXI)
class ListFragment : BaseBindingFragment<DiscussshiquxinxicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("拾取信息评论表",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("discussshiquxinxi","新增"))|| Utils.isAuthFront("discussshiquxinxi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_DISCUSSSHIQUXINXI).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("discussshiquxinxi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_DISCUSSSHIQUXINXI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun DiscussshiquxinxicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("discussshiquxinxi", requestParams)
        }.otherwise {
                mListModel.list("discussshiquxinxi", requestParams)
        }
    }
}